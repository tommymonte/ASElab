...
# 1 "Source/RIT\\../timer/timer.h" 1
# 14 "Source/RIT\\../timer/timer.h"
...
void RIT_IRQHandler (void)
{
 ...
 if((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<25)) == 0){ // J_select
  J_select++;
  switch(J_select){
   case 1:
 ...
 if((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<26)) == 0){ // J_down
  J_down++;
  switch(J_down){
   case 1:
    break;
 ...
 if((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<27)) == 0){ // J_left
  J_left++;
  switch(J_left){
   case 1:
 ...
 if((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<28)) == 0){ //J_right
  J_right++;
  switch(J_right){
   case 1:
    break;
 ...
 if((((LPC_GPIO_TypeDef *) ((0x2009C000UL) + 0x00020) )->FIOPIN & (1<<29)) == 0){ // J_up
  J_up++;
  switch(J_up){
   case 1:
    break;
   ...
   switch(EINT1_down){
    case 2:
     if(state == 0) {
      ...
      VAL = (((LPC_TIM_TypeDef *) ((0x40080000UL) + 0x10000) )->TC & (255 << 3)) >> 3;

      if( i == 0 && VAL%2==0) { // primo valore
        result = check_fibonacci(VETT, 4, M, VAL, BOUNDARY);
      } else if (i>0) {
        if(!((VAL%2==0 && VETT[i-1]%2==0) || (VAL%2!=0 && VETT[i-1]%2!=0))){
           result = check_fibonacci(VETT, 4, M, VAL, BOUNDARY);
         }
        else {
         result = 0; // se lo scarto a priori result = 0;
        }
        }
       if (result == 1) {
          i++;
          ...
          LED_Out(0xFF);
          cnt++; // incremento i valori scartati
         }
       }
     break;
    default:
